...
/* Modified by Sangtian Wang (wsangtian@vmware.com) */
...
class TDigest {
  ...
  public:
    ...
    static Value Sum(std::vector<Centroid> &centroids) {
        double s = 0;
        for (auto centroid : centroids) {
            s += centroid.weight() * centroid.mean();
        }
        return s;
    }

    Weight getWeight() { return weight(processed_); }

    Value getSum() { return Sum(processed_); }

    Value getMean() {
        Value w = getWeight();
        return w == 0 ? NAN : getSum() / getWeight();
    }
    ...
    TDigest(const TDigest &o)
        : TDigest(o.processed_, o.unprocessed_, o.compression_,
                  o.maxUnprocessed_, o.maxProcessed_) {}
    ...
    static inline Index unprocessedSize(Index size,
                                        Value compression) noexcept {
    ...
    bool isDirty() {
        return processed_.size() > maxProcessed_ ||
               unprocessed_.size() > maxUnprocessed_;
    }
    ...
    Value cdfProcessed(Value x) const {
        if (processed_.size() == 0) {
            ...
            if (x < min_) {
                ...
                // min_ and max_ are too close together to do any viable
                // interpolation
                return 0.5;
            } else {
            ...
            if (x <= mean(0)) {
                if (mean(0) - min_ > 0) {
                    return (x - min_) / (mean(0) - min_) * weight(0) /
                           processedWeight_ / 2.0;
                } else {
                    return 0;
                }
            }
            ...
            if (x >= mean(n - 1)) {
                if (max_ - mean(n - 1) > 0) {
                    return 1.0 - (max_ - x) / (max_ - mean(n - 1)) *
                                     weight(n - 1) / processedWeight_ / 2.0;
                } else {
                    return 1;
                }
            }
            ...
            auto iter = std::upper_bound(processed_.cbegin(), processed_.cend(),
                                         Centroid(x, 0), cc);
            ...
            return weightedAverage(cumulative_[i - 1], z2, cumulative_[i], z1) /
                   processedWeight_;
        }
    }
    ...
    // changing the t-digest the value will not represent the unprocessed values
    Value quantileProcessed(Value q) const {
        ...
        // if values were stored in a sorted array, index would be the offset we
        // are Weighterested in
        ...
        auto iter =
            std::lower_bound(cumulative_.cbegin(), cumulative_.cend(), index);
  ...
  private:
    ...
    void mergeUnprocessed(const std::vector<const TDigest *> &tdigests) {
        ...
        for (auto &td : tdigests) {
            unprocessed_.insert(unprocessed_.end(), td->unprocessed_.cbegin(),
                                td->unprocessed_.cend());
    ...
    // merges unprocessed_ centroids and processed_ centroids together and
    // processes them when complete, unprocessed_ will be empty and processed_
    // will have at most maxProcessed_ centroids
    inline void process() {
        CentroidComparator cc;
        std::sort(unprocessed_.begin(), unprocessed_.end(), cc);
        auto count = unprocessed_.size();
        unprocessed_.insert(unprocessed_.end(), processed_.cbegin(),
                            processed_.cend());
        std::inplace_merge(unprocessed_.begin(), unprocessed_.begin() + count,
                           unprocessed_.end(), cc);
    ...
    /**
     * Converts a quantile into a centroid scale value.  The centroid scale is
     * nomin_ally the number k of the centroid that a quantile point q should
     * belong to.  Due to round-offs, however, we can't align things perfectly
     * without splitting points and sorted.  We don't want to do that, so we
     * have to allow for offsets. In the end, the criterion is that any quantile
     * range that spans a centroid scale range more than one should be split
     * across more than one centroid if possible.  This won't be possible if the
     * quantile range refers to a single point or an already existing centroid.
     * <p/> This mapping is steep near q=0 or q=1 so each centroid there will
     * correspond to less q range.  Near q=0.5, the mapping is flatter so that
     * sorted there will represent a larger chunk of quantiles.
     *
    ...
    inline Value integratedQ(Value k) const {
        return (std::sin(std::min(k, compression_) * M_PI / compression_ -
                         M_PI / 2) +
                1) /
               2;
    }
     ...
     * flips the order of the variables if <code>x2</code> is greater than
     * <code>x1</code>.
     */
